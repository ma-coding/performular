...
export class RendererDirective implements OnDestroy {
    ...
    private _createComponent(field: Abstract): void {
        const form: FormComponent = <any>field.form;
        // const wrapTemplate: TemplateDirective | undefined = form.getTemplate(field.id, field.fieldDef, 'wrap');

        let componentRef: ComponentRef<any>;
        // if (wrapTemplate) {
        //     componentRef = this._renderWrappedComponent(field, wrapTemplate);
        // } else {
        componentRef = this._renderDefaultComponent(field);
        // }

        field.registerFramework(componentRef.location, componentRef.instance);
        this._renderTemplate(field, componentRef, 'before');
        this._renderTemplate(field, componentRef, 'after');
    }

    // private _renderWrappedComponent(field: Abstract, wrapTemplate: TemplateDirective): ComponentRef<any> {
    ...
    private _renderTemplate(field: Abstract, componentRef: ComponentRef<any>, position: TemplatePosition): void {
        if (field.form) {
            const template: TemplateDirective | undefined = field.form.getTemplate(field.id, field.fieldDef, position);
            if (template) {
